...
public class RstDetail extends Activity {
		...
		GsonRequest<JsonObject> req = new GsonRequest<JsonObject>(method, "/read_comments",
				JsonObject.class, params, new Listener<JsonObject>() {
					...
					public void onResponse(JsonObject comments) {
						...
						// ScrollView comments = (ScrollView) findViewById(R.id.comments);

						LinearLayout comment_component = (LinearLayout) findViewById(R.id.comment_component);
	...
	private void setRestaurantDetailToTextView(final RestaurantDetail rst) {
		TextView name = (TextView) findViewById(R.id.rst_detail_name);
		name.setText(rst.getRestaurantName());

		// 食べログのurlをWebView内で開くためのもの
		// TextView url = (TextView) findViewById(R.id.rst_detail_data_tabelog);
		Button url = (Button) findViewById(R.id.rst_detail_data_tabelog);
		...
		// 以下、埋め込み作業が続く
		TextView category = (TextView) findViewById(R.id.rst_detail_data_category);
		category.setText(rst.getCategory());

		TextView dinner = (TextView) findViewById(R.id.rst_detail_data_dinner);
		dinner.setText(rst.getDinnerPrice());

		TextView lunch = (TextView) findViewById(R.id.rst_detail_data_lunch);
		lunch.setText(rst.getLunchprice());

		TextView situation = (TextView) findViewById(R.id.rst_detail_data_situation);
		situation.setText(rst.getSituation());

		TextView address = (TextView) findViewById(R.id.rst_detail_data_address);
		address.setText(rst.getAddress());

		TextView station = (TextView) findViewById(R.id.rst_detail_data_station);
		station.setText(rst.getStation());

		TextView tel = (TextView) findViewById(R.id.rst_detail_data_tel);
		tel.setText(rst.getTel());
		Linkify.addLinks(tel, Linkify.PHONE_NUMBERS);

		TextView bh = (TextView) findViewById(R.id.rst_detail_data_businesshour);
		bh.setText(rst.getBusinesshours());

		TextView holiday = (TextView) findViewById(R.id.rst_detail_data_holiday);
		holiday.setText(rst.getHoliday());

		RatingBar difficulty = (RatingBar) findViewById(R.id.detail_difficulty);
